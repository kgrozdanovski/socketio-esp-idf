...
esp_err_t socketio_begin(const sio_client_t *sio_client, const uint8_t connect_retries)
{
    esp_err_t hanshake_result = ESP_OK;
    int retried = 0;

    while (retried < connect_retries)
    {
        hanshake_result = socketio_http_handshake(sio_client);
        if (hanshake_result != ESP_OK)
        {
    ...
    ESP_LOGI(SIO_TAG, "socketio_begin: failed after %d connection retries, last attempt result: %d",
             retried, hanshake_result);
    return hanshake_result;
}
